import { recommendationFromScore } from "./recommendation.ts";
import type {
  Answer,
  BuyDecision,
  Confidence,
...
function validateUserContextValue(value: unknown): ValidationResult<UserContext> {
  ...
  if (value.buyingCountry !== undefined && typeof value.buyingCountry !== "string") {
  ...
  if (value.sellerCurrency !== undefined && typeof value.sellerCurrency !== "string") {
...
function validateObjectAnalysis(value: unknown): ValidationResult<ObjectAnalysis> {
  ...
  for (const key of [
    ...
    if (typeof value[key] !== "string") return fail(`analysis.${key} must be a string.`);
  }
  for (const key of [
    "conditionObservations",
    "qualityChecklist",
    ...
    if (!isStringArray(value[key])) return fail(`analysis.${key} must be a string array.`);
  }
  if (!isOneOf(value.confidence, confidenceValues)) return fail("analysis.confidence is invalid.");
...
function validateBuyDecision(value: unknown): ValidationResult<BuyDecision> {
  ...
  if (!isStringArray(value.reasons)) return fail("decision.reasons must be a string array.");
  if (!isStringArray(value.risks)) return fail("decision.risks must be a string array.");
...
function validateModelDecision(
  ...
  if (!isFiniteNumber(value.worthBringingHomeScore)) return fail("model decision score must be a number.");
  if (!isFiniteNumber(value.suggestedMaxPrice) || value.suggestedMaxPrice < 0) {
...
function validateOption(value: unknown): ValidationResult<ReportImprovementFieldOption> {
  ...
  if (typeof value.labelKey !== "string") return fail("option.labelKey must be a string.");
...
function validateImprovementField(
  ...
  for (const key of ["id", "key", "labelKey"] as const) {
    if (typeof value[key] !== "string") return fail(`improvement field ${key} must be a string.`);
  }
  ...
  if (typeof value.required !== "boolean") return fail("improvement field required must be a boolean.");
  if (!isOneOf(value.priority, priorityValues)) return fail("improvement field priority is invalid.");
...
function validateImprovementForm(value: unknown): ValidationResult<ReportImprovementForm> {
  ...
  for (const key of ["id", "reportId", "titleKey", "descriptionKey", "createdAt"] as const) {
    if (typeof value[key] !== "string") return fail(`improvementForm.${key} must be a string.`);
  }
...
function validateFollowUpQuestion(value: unknown): ValidationResult<FollowUpQuestion> {
  ...
  for (const key of ["id", "question", "reason"] as const) {
    if (typeof value[key] !== "string") return fail(`followUpQuestion.${key} must be a string.`);
  }
  ...
  if (typeof value.answered !== "boolean") return fail("followUpQuestion.answered must be a boolean.");
  if (typeof value.skipped !== "boolean") return fail("followUpQuestion.skipped must be a boolean.");
...
export function validateGenerateInitialReportRequest(
  ...
  if (value.previousQuestions !== undefined) {
    if (!Array.isArray(value.previousQuestions)) return fail("previousQuestions must be an array.");
...
function validateReportImprovementSubmission(
  value: unknown,
  ...
  if (typeof value.reportId !== "string") return fail("submission.reportId must be a string.");
  if (!isRecord(value.values)) return fail("submission.values must be an object.");
  for (const submittedValue of Object.values(value.values)) {
    if (!isFieldValue(submittedValue)) return fail("submission.values contains invalid field values.");
  }
...
function validateAnswer(value: unknown): ValidationResult<Answer> {
  ...
  if (typeof value.questionId !== "string") return fail("answer.questionId must be a string.");
...
export function validateGenerateUpdatedReportRequest(
  ...
  if (value.newPhotoStoragePaths !== undefined && !isStringArray(value.newPhotoStoragePaths)) {
...
export function validateObjectReport(value: unknown): ValidationResult<ObjectReport> {
  ...
  if (!isOneOf<ReportStatus>(value.status, reportStatusValues)) return fail("ObjectReport.status is invalid.");
  if (!isOneOf<ReportMode>(value.mode, reportModeValues)) return fail("ObjectReport.mode is invalid.");
  ...
  if (!decision.ok) return fail(decision.message);
  if (!Array.isArray(value.followUpQuestions)) return fail("ObjectReport.followUpQuestions must be an array.");
  ...
  if (
    value.userDecision !== undefined &&
    !isOneOf<UserDecision>(value.userDecision, userDecisionValues)
  ) {
  ...
  if (typeof value.createdAt !== "string") return fail("ObjectReport.createdAt must be a string.");
  if (typeof value.updatedAt !== "string") return fail("ObjectReport.updatedAt must be a string.");
