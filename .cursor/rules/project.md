...
- The **`ObjectReport`** is the **single source of truth** for valuation. UI reads from it; chat and the optional `ReportImprovementForm` are used to gather missing information and explain the report.
- Two clear feature folders:
  - `src/features/chat` — chat UI state only (messages, composer, attached photos, open improvement-form state). No valuation logic. Owns `chat.reducer.ts` + `chat.provider.tsx` (`useChat()`).
  - `src/features/report` — `ObjectReport` types, `ReportImprovementForm` types, `report.reducer.ts` + `report.provider.tsx` (`useLatestReport()`, `useReportById(id)`), and the deterministic async mock services `report.service.ts` and `report.updateService.ts`.
- Components are presentational and typed. Services are pure and deterministic; update-producing services are **async** (return `Promise<ObjectReport>`). No fetch / IO inside components.
- **Type-file dependency direction:** `chat.types.ts` imports `FollowUpQuestion`, `Answer`, `ReportImprovementForm`, and related submission types from `report.types.ts`. The report engine never imports from `chat.types.ts`.
...
- The mock report engine returns translation **keys** for `FollowUpQuestion.question` / `reason`, `ReportImprovementForm` titles, field labels, helpers, options, and similar copy. The UI resolves them with `t()` at render time.
...
- After the initial report, prefer showing an optional **Improve this report in 30 seconds** card that opens a short object-specific `ReportImprovementForm`. Generate fields from missing evidence and unanswered follow-up questions. Keep it short: usually 2-4 fields and fillable in about 30 seconds.
- Any new information from the user (structured form submission, text answer, skipped question, new photos, updated context) goes through `report.updateService.applyImprovementSubmission`, `applyAnswer` (with an `Answer` carrying optional `contextPatch`), `applyQuestionSkip`, or `applyPhotos`, and produces a **new** `ObjectReport`. Never mutate report fields from components. There is no separate `applyContextUpdate`.
- A `ReportImprovementForm` is a deterministic projection of the latest report, not a persisted second source of truth. Submitting it updates the current `ObjectReport` exactly once and then posts an assistant summary message in chat.
...
- Bubble-style follow-up questions remain supported, but most follow-up prompts should be represented as generated form fields when possible. Only post question bubbles for fallback clarification, questions that do not fit the short form, or users who continue in chat.
